...
--- VERSION ---
    ...
    public IReadOnlyList<PlaylistSummaryDto> GetPlaylists(string username)
    {
        ...
        lock (_syncRoot)
        {
            EnsureLikedSongsPlaylist(userKey);
            var libraryByFile = _musicLibraryService.GetLibrary().ToDictionary(item => item.FileName, StringComparer.OrdinalIgnoreCase);
            var playlists = _data[userKey];

            return playlists
                .OrderByDescending(item => string.Equals(item.Name, "Liked Songs", StringComparison.OrdinalIgnoreCase))
    ...
    public IReadOnlyList<PlaylistSummaryDto> DeletePlaylist(string username, string playlistId)
    {
        ...
        lock (_syncRoot)
        {
            ...
            var target = playlists.FirstOrDefault(item => string.Equals(item.Id, safeId, StringComparison.OrdinalIgnoreCase));
            if (target != null && string.Equals(target.Name, "Liked Songs", StringComparison.OrdinalIgnoreCase))
            {
    ...
    public IReadOnlyList<PlaylistSummaryDto> ToggleLikedSong(string username, string fileName)
    {
        ...
        lock (_syncRoot)
        {
            EnsureLikedSongsPlaylist(userKey);
            var playlists = _data[userKey];
            var likedPlaylist = playlists.First(item => string.Equals(item.Name, "Liked Songs", StringComparison.OrdinalIgnoreCase));

            if (likedPlaylist.Songs.Contains(safeFile, StringComparer.OrdinalIgnoreCase))
            {
                likedPlaylist.Songs.RemoveAll(s => string.Equals(s, safeFile, StringComparison.OrdinalIgnoreCase));
            }
            else
            {
                likedPlaylist.Songs.Add(safeFile);
            }
    ...
    private void EnsureLikedSongsPlaylist(string userKey)
    {
        if (!_data.TryGetValue(userKey, out var playlists))
        {
            playlists = [];
        ...
        if (!playlists.Any(item => string.Equals(item.Name, "Liked Songs", StringComparison.OrdinalIgnoreCase)))
        {
            playlists.Insert(0, new PlaylistRecord
            {
                Id = "liked-songs-" + userKey,
                Name = "Liked Songs",
                ImageUrl = "",
                Songs = []
            });
    ...
    private Dictionary<string, List<PlaylistRecord>> Load()
    {
        ...
        try
        {
            ...
            {
                var playlists = dict[key];
                if (!playlists.Any(p => string.Equals(p.Name, "Liked Songs", StringComparison.OrdinalIgnoreCase)))
                {
                    playlists.Insert(0, new PlaylistRecord
                    {
                        Id = "liked-songs-" + key,
            ...
            if (dirty)
            {
                ...
                File.WriteAllText(_storagePath, newJson);
            }
